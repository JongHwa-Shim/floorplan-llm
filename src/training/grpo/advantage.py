...
GDPOTrainer._apply_token_credit_assignment()에서 호출되는
핵심 어드밴티지 계산 파이프라인.
...
from src.training.grpo.rewards.credit_assignment import apply_token_credit_assignment
...
def compute_token_advantages(
    ...
    for i in range(B_local):
        ...
        for k, (name, cfg) in enumerate(zip(reward_names, reward_cfgs)):
            ...
            if cfg.get("credit_assignment", False):
